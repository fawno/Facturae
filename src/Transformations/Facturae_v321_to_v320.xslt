...
<xsl:stylesheet version="1.0"
	xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fe321="http://www.facturae.es/Facturae/2014/v3.2.1/Facturae"
	xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="fe321 xs">
	...
	<xsl:template name="tmpl10_AmountType">
		...
		<xsl:for-each select="$input/node()">
			<xsl:if test="boolean(self::TotalAmount)">
				<TotalAmount>
					<xsl:value-of
						select="format-number(number(string(.)), '#0.00', 'decimal')" />
				</TotalAmount>
			</xsl:if>
		...
		<xsl:for-each select="$input/node()">
			<xsl:if test="boolean(self::ExchangeRate)">
				<ExchangeRate>
					<xsl:value-of
						select="format-number(number(string(.)), '#0.00', 'decimal')" />
					<!-- <xsl:value-of select='format-number( round(100*(number(string(.)))) 
						div 100 ,"##0.00" )' /> -->
				</ExchangeRate>
			</xsl:if>
	...
	<xsl:template name="tmpl22_DiscountType">
		...
		<xsl:for-each select="$input/node()">
			...
			<xsl:if test="boolean(self::DiscountRate)">
				<DiscountRate>
					<xsl:value-of
						select="format-number(number(string(.)), '#0.0000', 'decimal')" />
				</DiscountRate>
			</xsl:if>
			<xsl:if test="boolean(self::DiscountAmount)">
				<DiscountAmount>
					<xsl:value-of
						select="format-number(number(string(.)), '#0.000000', 'decimal')" />
				</DiscountAmount>
			</xsl:if>
		</xsl:for-each>
	</xsl:template>
		...
		<xsl:for-each select="$input/node()">
			<xsl:if test="boolean(self::AdministrativeCentre)">
				<AdministrativeCentre>
					...
					<xsl:if
						test="boolean(self::AdministrativeCentre/LogicalOperationalPoint)">
	...
	<xsl:template match="/">
		<fe:Facturae xmlns:fe="http://www.facturae.es/Facturae/2009/v3.2/Facturae"
			...
			<xsl:attribute name="xsi:schemaLocation"
				...
				<xsl:value-of
				select="'http://www.facturae.es/Facturae/2009/v3.2/Facturae'" />
			</xsl:attribute>
			<xsl:for-each select="fe321:Facturae">
				<xsl:variable name="var1_FileHeader"
					select="*[local-name()='FileHeader' and namespace-uri()='']" />
				<xsl:variable name="var2_Parties"
					select="*[local-name()='Parties' and namespace-uri()='']" />
				<xsl:variable name="var3_SellerParty"
					select="$var2_Parties/*[local-name()='SellerParty' and namespace-uri()='']" />
				<xsl:variable name="var4_BuyerParty"
					select="$var2_Parties/*[local-name()='BuyerParty' and namespace-uri()='']" />
				<FileHeader xmlns="">
					<SchemaVersion>
							...
							<PaymentDetails>
								<xsl:call-template name="tmplInstallment">
									<xsl:with-param name="input"
										select="$var1_FileHeader/Installment" />
								</xsl:call-template>
							</PaymentDetails>
				...
				<Parties xmlns="">
					<SellerParty>
						<TaxIdentification>
							<xsl:call-template name="tmpl2_TaxIdentificationType">
								<xsl:with-param name="input"
									select="$var3_SellerParty/TaxIdentification" />
							</xsl:call-template>
						</TaxIdentification>
					...
					<BuyerParty>
						<TaxIdentification>
							<xsl:call-template name="tmpl2_TaxIdentificationType">
								<xsl:with-param name="input"
									select="$var4_BuyerParty/TaxIdentification" />
							</xsl:call-template>
						</TaxIdentification>
							...
							<Items>
								<xsl:for-each select="Items/InvoiceLine">
									<InvoiceLine>
										...
										<UnitPriceWithoutTax>
											<xsl:value-of
												select="format-number(number(string(UnitPriceWithoutTax)), '#0.000000', 'decimal')" />
										</UnitPriceWithoutTax>
										<TotalCost>
											<xsl:value-of
												select="format-number(number(string(TotalCost)), '#0.000000', 'decimal')" />
										</TotalCost>
										...
										<GrossAmount>
											<xsl:value-of
												select="format-number(number(string(GrossAmount)), '#0.000000', 'decimal')" />
										</GrossAmount>
										<xsl:for-each select="TaxesWithheld">
											<TaxesWithheld>
												<xsl:call-template name="tmpl18_TaxesType">
